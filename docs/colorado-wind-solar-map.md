...
- **Brutalist overlays:** semi-transparent `#111111` fill over Colorado, **white** outline (`#ffffff`), solar and wind **SVG icons** with a subtle warm/cool tint (`#ffe6d4` / `#dceaf4` in `lib/map-plant-icons.ts`, registered via `map.addImage` on symbol layers).
...
- **Solar:** `symbol` layer with `plant-solar` icon, `kind === 'sun'`.
- **Wind:** `symbol` layer with `plant-wind` icon, `kind === 'wnd'`.
- **Icons:** `lib/map-plant-icons.ts` (brutalist SVG rasterized to `ImageBitmap`, then `map.addImage`).
...
| Plant icons | `lib/map-plant-icons.ts` |
