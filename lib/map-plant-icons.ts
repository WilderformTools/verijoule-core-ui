/** Brutalist white plant markers for MapLibre (`#ffffff` on transparent). */

const STROKE = "#ffffff";

/** Square hub + thick cardinal rays — reads as solar at small map sizes. */
const SOLAR_ICON_BODY = `
  <rect x="18" y="18" width="12" height="12" fill="${STROKE}"/>
  <rect x="22" y="3" width="4" height="11" fill="${STROKE}"/>
  <rect x="22" y="34" width="4" height="11" fill="${STROKE}"/>
  <rect x="3" y="22" width="11" height="4" fill="${STROKE}"/>
  <rect x="34" y="22" width="11" height="4" fill="${STROKE}"/>
  <rect x="8" y="8" width="4" height="9" fill="${STROKE}" transform="rotate(-45 10 12.5)"/>
  <rect x="36" y="8" width="4" height="9" fill="${STROKE}" transform="rotate(45 38 12.5)"/>
  <rect x="8" y="31" width="4" height="9" fill="${STROKE}" transform="rotate(45 10 35.5)"/>
  <rect x="36" y="31" width="4" height="9" fill="${STROKE}" transform="rotate(-45 38 35.5)"/>
`;

/** Hub, mast, and three square-ended blades at 120°. */
const WIND_ICON_BODY = `
  <rect x="21" y="30" width="6" height="14" fill="${STROKE}"/>
  <rect x="20" y="20" width="8" height="8" fill="${STROKE}"/>
  <line x1="24" y1="24" x2="24" y2="5" stroke="${STROKE}" stroke-width="4" stroke-linecap="square"/>
  <line x1="24" y1="24" x2="40.5" y2="33.5" stroke="${STROKE}" stroke-width="4" stroke-linecap="square"/>
  <line x1="24" y1="24" x2="7.5" y2="33.5" stroke="${STROKE}" stroke-width="4" stroke-linecap="square"/>
`;
